...
impl ExprProcessor {
    ...
    fn check_static_function_call(
        ...
        let info = self
            ...
            .get(expr_id)
            .expect("Function call info not found");
        ...
        if self.type_store.reshape(
            &info.function_type,
            &target_function_type_info.function_type,
        ) {
            self.print_type(info.function_type, "info.function_type");
            self.print_type(
                target_function_type_info.function_type,
                "target_info.function_type",
            );
            let expr_var = self.lookup_type_var_for_expr(expr_id);
            if self.type_store.unify(&expr_var, &info.result) {
                for (arg, gen_arg) in args.iter().zip(info.args.iter()) {
    ...
    fn check_tuple_field_access(
        ...
        let tuple_expr_ty = self.type_store.get_type(&tuple_var);
        let ok = {
            match tuple_expr_ty {
                Type::TypeArgument(_) => {
                    let var = self.type_store.add_type(Type::TupleFieldIndexable(index));
                    self.type_store.merge(&var, &tuple_var);
                    true
                }
                Type::TupleFieldIndexable(i) => {
                    let var = self
                        .type_store
                        .add_type(Type::TupleFieldIndexable(std::cmp::max(index, i)));
                    self.type_store.merge(&var, &tuple_var);
                    true
                }
                Type::Tuple(items) => {
        ...
        if !ok {
            let expected_type = format!("<tuple with at least {} items>", index + 1);
        ...
        for (expr_id, expr_info) in &program.exprs {
            ...
            match &expr_info.expr {
                ...
                Expr::StaticFunctionCall(_, args) => {
                    ...
                    self.function_call_info_map.insert(*expr_id, info);
